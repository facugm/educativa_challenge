...
sub ingresar {
	...
	my $repetido = $db->prepare("SELECT id_alumno FROM usuarios_cursos WHERE id_curso = ?");
	...
	my @profesores = $db->selectall_array("SELECT c.id_docente, c.id_curso FROM cursos c;");
	foreach (@profesores) {
		if (@$_[0] == $alum and @$_[1] == $curso) {print "La persona a ingresar es el docente del curso seleccionado\n"; $flag = 0;}
	}

	# Traigo conteo de gente inscripta y nro de cupo del curso a inscribir y comparo si son iguales
	my $cupo_disp = $db->prepare("SELECT COUNT(uc.id_alumno), c.cupo FROM usuarios_cursos uc INNER JOIN cursos c ON uc.id_curso = c.id_curso WHERE uc.id_curso = ?");
	...
	if($flag) {
	my $cargar_alumno = $db->prepare("INSERT INTO usuarios_cursos(id_curso,id_alumno) VALUES(?,?)");
...
sub borrar {
	...
	my $eliminar = $db->prepare("DELETE FROM usuarios_cursos WHERE id_curso = ? AND id_alumno = ?");
	# Si no se borró nada, notifica al usuario
	if ($eliminar->execute($curso, $alum) != 0E0) {
		print "Alumno $alum eliminado del curso $curso\n";}
	else {
		print "ERROR: Imposible de borrar. Alumno $alum no está inscripto en el curso $curso\n";
	}
}