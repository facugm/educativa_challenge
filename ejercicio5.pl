...
my $stmt = $dbh->prepare("INSERT IGNORE INTO usuarios(id_usuario,nombre,apellido)
						VALUES(?,?,?)");
my $stmt2= $dbh->prepare("REPLACE INTO usuarios_cursos(id_curso,id_alumno)
						VALUES(?,?)");
...
my $j=0;
# Preparo query
$stmt = $dbh->prepare("INSERT IGNORE INTO cursos(id_curso,nombre,cupo,id_docente)
	   				VALUES(?,?,?,?)");
...
sub imprimir {
	...
	my $stmt = $db->prepare("SELECT u.id_usuario, c.id_curso, u.nombre, u.apellido, c.nombre
