...
my $response1 =
  $ua1->get('http://packages.educativa.com/samples/usuarios.json');
...
while ( $i < scalar @users ) {
    print "$users[$i]->{id}\t$users[$i]->{nombre}\t$users[$i]->{apellido} "
      . "@{$users[$i]->{id_curso}}\n";
    $i++;
}
...
while ( $j < scalar @cursos ) {
    print
"$cursos[$j]->{id}\t$cursos[$j]->{nombre}\t$cursos[$j]->{id_docente}\t$cursos[$j]->{cupo}\n";
    $j++;
}
